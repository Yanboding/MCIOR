...
class Value(gpytorch.models.ExactGP, nn.Module):
    ...
    def __init__(self, state_dim, action_dim, gp_likelihood=None):
        # action_dim = fisher_num_inputs is same as svd_low_rank, because of the linear approximation of the Fisher kernel through FastSVD.
        ...
        state_kernel_active_dims = torch.tensor(list(range(self.state_element_num)))
        # [NN_num_outputs, GP_input.shape[1]-1] indices of GP's input correspond to the fisher_kernel
        fisher_kernel_active_dims = torch.tensor(list(range(self.state_element_num, self.action_element_num + self.state_element_num)))
